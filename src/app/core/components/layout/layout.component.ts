...
export class LayoutComponent implements OnInit {
  private static readonly COMPACT_MENU_STORAGE_KEY = 'expense-tracker.compact-menu';
  ...
  ngOnInit(): void {
    this.loadCompactMenuPreference();
  ...
  toggleIconsOnly() {
    const nextState = !this.isCompactMenuEnabled();
    this.isCompactMenuEnabled.set(nextState);
    this.saveCompactMenuPreference(nextState);
  }
  ...
  private loadCompactMenuPreference() {
    try {
      const isCompact = localStorage.getItem(LayoutComponent.COMPACT_MENU_STORAGE_KEY) === 'true';
      this.isCompactMenuEnabled.set(isCompact);
    } catch {
      this.isCompactMenuEnabled.set(false);
    }
  }

  private saveCompactMenuPreference(isCompact: boolean) {
    try {
      localStorage.setItem(LayoutComponent.COMPACT_MENU_STORAGE_KEY, String(isCompact));
    } catch {
